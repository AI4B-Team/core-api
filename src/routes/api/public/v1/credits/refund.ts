...
export const Route = createFileRoute("/api/public/v1/credits/refund")({
  server: {
    handlers: {
      /** Refunds are ledger entries, never deletions. */
      POST: async ({ request }) => {
        ...
        const { data, error } = await db.rpc("consume_credits", {
          ...
          _reference: { ...(input.reference ?? {}), refund: true },
        });
        if (error) return apiError("refund_failed", 400, { detail: error.message });
